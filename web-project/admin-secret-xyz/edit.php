...
$fields['id'] = $destination['id'];
$fields['title'] = $destination['title'];
$fields['description'] = $destination['description'];
$fields['nights'] = $destination['nights'];
$fields['days'] = $destination['days'];
$fields['cost'] = $destination['cost'];
$fields['current_image'] = $destination['image'];
$fields['rating'] = $destination['rating'];

// Process highlights and itinerary
if (!empty($destination['highlights'])) {
    $fields['highlights'] = explode("\n", $destination['highlights']);
}
if (!empty($destination['itinerary'])) {
    $fields['itinerary'] = explode("\n", $destination['itinerary']);
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['size'] > 0) {
        ...
        if (move_uploaded_file($image_tmp_name, $image_path)) {
            ...
            if ($fields['current_image'] && $fields['current_image'] != $image_name) {
                @unlink("../uploads/" . $fields['current_image']);
            }
        } else {
            $error = "Failed to upload image.";
        }
    } else {
        $fields['image'] = $fields['current_image'];
    }
    
    // Update destination
    if ($fields['title'] && $fields['description'] && $fields['nights'] && 
        $fields['days'] && $fields['cost']) {
        ...
        $query = "UPDATE destinations SET 
                 title = ?, description = ?, nights = ?, days = ?, 
                 cost = ?, image = ?, highlights = ?, itinerary = ?, rating = ?
                 WHERE id = ?";
        
        $stmt = $conn->prepare($query);
        $stmt->bind_param("ssiidsssdi", 
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <div class="form-section">
                ...
                <div class="form-group">
                    ...
                    <?php if ($fields['current_image']): ?>
                        <div class="current-image">
                            <p>Current Image:</p>
                            <img src="../uploads/<?= htmlspecialchars($fields['current_image']) ?>" alt="Current destination image">
                        </div>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <button type="button" class="submit"><a href="index.php">Back to List</a></button>
        </form>
    </div>
